...
varda_long_vt_total <- varda_long_red %>% 
  select(!varve_thick & !dark_lamin_thick) %>% 
  filter(!is.na(light_lamin_thick)) %>% 
  rename(varve_thick = light_lamin_thick) #the modified mgcv2tapas_krzypl2() function requires the variable is calles "varve_thick"
...
pd_df_vt_light_mgcv <- pd_df_prep %>%
  imap_dfr(~ add_lake_name(.x, .y)) %>% 
  mutate(age_CE = 1950-age) %>% 
  rename(age_BP = age, light_lamin_thick = varve_thickAR)

write_csv(pd_df_vt_light_mgcv, "data/pd_df_vt_light_mgcv.csv")